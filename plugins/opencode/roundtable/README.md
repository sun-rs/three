# roundtable-opencode (MVP, directory keeps `roundtable`)

A true OpenCode plugin for roundtable-first orchestration using OpenCode's native plugin/runtime APIs.

## What it provides

- `roundtable_native_roundtable` - run multi-round discussion (`rounds >= 1`) on one shared topic.
...
ln -sf "$(git rev-parse --show-toplevel)/plugins/opencode/roundtable/index.js" .opencode/plugins/roundtable-opencode.js
```
...
ln -sf "$(git rev-parse --show-toplevel)/plugins/opencode/roundtable/index.js" ~/.config/opencode/plugins/roundtable-opencode.js
```
...
- Per-agent child session IDs persist at `<worktree>/.roundtable/opencode-session-store.json`.
- `roundtable_native_roundtable` rules:
  ...
  - Round evidence persists to `.roundtable/roundtable-artifacts/...` by default (`persist_round_artifacts=true`).
